...
public class CampaignStage7 extends CampaignStage1 {
    /**
     * Constructor to setup the game and the GUI components
     *
     * @param name
     */
    protected CampaignStage7(String name) {
        super(name);
    }
    @Override
    protected void PlayGame(String name1, String name2) {
        ...
        JOptionPane.showMessageDialog(null,"Chào mừng bạn đến với Stage "+Campaign.stage);
    ...
    public void initGame() {
        ...
        board[ROWS/2-1][COLS/2-1] = Seed.NOUGHT;
        board[ROWS/2][COLS/2] = Seed.NOUGHT;
        board[ROWS/2+1][COLS/2+1] = Seed.NOUGHT;
    ...
    public void updateGame(Seed theSeed, int rowSelected, int colSelected) {
        if (hasWon(theSeed, rowSelected, colSelected)) {  // check for win
            ...
            if(theSeed == Seed.CROSS){
                winner = Player1Name;
                JOptionPane.showMessageDialog(null,"Chúc mửng "+Player1Name+" đã hoàn thành campaign");
            }
            else {
                winner = "nought";
